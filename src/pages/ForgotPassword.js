...
const ForgotPassword = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.post(`${process.env.REACT_APP_API_URI}/api/accounts/forgot-password`, { email: email });
  ...
  return (
    <section className="flex min-h-[100vh] flex-1 flex-col justify-center px-6 py-12 lg:px-8">
      <div className="sm:mx-auto sm:w-full sm:max-w-sm">
        ...
        {isSending &&
          <p className="text-gray-600 dark:text-gray-400">If an account associated with the email address is found, we will send instructions to reset the password to the email address.</p>
        }
        {!isSending &&
          <>
